...
import io.github.lxgaming.clearlag.commands.AbstractCommand;
...
public final class CommandManager {
    ...
    public static CommandResult process(AbstractCommand abstractCommand, CommandSource commandSource, String message) {
        ...
        ClearLag.getInstance().getLogger().debug("Processing {} for {}", command.get().getPrimaryAlias().orElse("Unknown"), commandSource.getName());
        
        try {
            return command.get().execute(commandSource, arguments.get());
        } catch (Exception ex) {
            ClearLag.getInstance().getLogger().error("Encountered an error processing {}::process", "CommandManager", ex);
    ...
    public static boolean registerCommand(Class<? extends AbstractCommand> commandClass) {
        if (getCommandClasses().contains(commandClass)) {
            ClearLag.getInstance().getLogger().warn("{} has already been registered", commandClass.getSimpleName());
            return false;
        }
        ...
        Sponge.getCommandManager().register(ClearLag.getInstance().getPluginContainer(), command.get(), command.get().getAliases().toArray(new String[0]));
    ...
    public static boolean registerAlias(AbstractCommand command, String alias) {
        if (Toolbox.containsIgnoreCase(command.getAliases(), alias)) {
            ClearLag.getInstance().getLogger().warn("{} has already been registered for {}", alias, command.getClass().getSimpleName());
            return false;
        }
